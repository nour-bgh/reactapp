  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
      <div className="mb-10 rounded-[2rem] border border-slate-200 bg-white p-8 shadow-2xl shadow-slate-300/10">
        <h1 className="text-3xl font-bold text-slate-950">Mes favoris</h1>
        <p className="mt-2 text-slate-500">Les logements que vous avez aimés.</p>
      </div>
      ...
      <div className="grid gap-6">
        {items.length > 0 ? items.map(item => (
            ...
            <div className="p-6 sm:flex-1">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                <button
                  ...
                  className="flex items-center gap-2 rounded-full border border-rose-300 px-5 py-2 text-sm font-semibold text-rose-600 transition hover:bg-rose-50"
                >
          ...
          <p className="rounded-[2rem] border border-slate-200 bg-white p-8 text-slate-600">Vous n'avez aucun favori pour le moment.</p>
        )}
      </div>
    </div>
  );
